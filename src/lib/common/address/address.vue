    ...
    keep-alive
      el-row.address-row(:gutter="0")
        el-col.address-col(:span="6" style="padding:20px 0 20px 8px;")
          keep-alive
            kalix-nav-menu.address-menu(title="组列表" bizKey="addressGroup"
              ...
              template(slot="menuItemSlot")
                el-menu-item(v-if="menuItems.length" v-bind:index="itemIndex.toString()" v-for="(item, itemIndex) in menuItems" v-bind:key="item.id" )
                  ...
                  span(slot="title")
                    i(v-bind:class="item.iconCls")
                    | {{item.groupName}}
                  kalix-menu-tool(v-if="!item.default" v-bind:btnList="groupBtnList" v-bind:item="item"
        ...
        el-col.address-col(:span="18" style="position:relative;height:100%;box-sizing: border-box;")
          keep-alive
            kalix-table.address-wrapper(bizKey="address" title="通讯录" bizSearch="CommonAddressSearch"
...
<script type="text/ecmascript-6">
  ...
  import {AddressGroupURL, AddressURL} from '../config.toml'
  import {addressBtnList} from './config'
  import {groupBtnList} from './groupIndex'
  // import {registerComponent} from '@/api/register'

  // registerComponent(AddressComponent)
  export default {
    name: 'kalix-common-address',
    props: {
      btnList: {   //  table中按钮数组
        type: Array,
        required: false,
    ...
    data() {
      return {
        ...
        paramStr: `{'userId': ` + this.$KalixCatch.get('id') + `}`,
        userId: this.$KalixCatch.get('id'),
